...
public class ItemService {
    ...
    public Long saveItme(ItemFormDto itemFormDto, List<MultipartFile> itemImgFileLIst)throws  Exception {
        //상품 등록
        ...
        for (int i = 0; i < itemImgFileLIst.size(); i++) {
            ...
            itemImgService.savedItemImg(itemImg, itemImgFileLIst.get(i));// 상품 이미지 정보를 저장한다.
        }
        return item.getId();
    }
    ...
    public Long updateItem(ItemFormDto itemFormDto, List<MultipartFile> itemimgFileList)throws Exception {// 상품 수정
        ...
        for (int i = 0; i < itemImgIds.size(); i++) {
            itemImgService.updateItemImg(itemImgIds.get(i), itemimgFileList.get(i)); // 상품 이미지 아이디를 업데이트하기 위해서 상품이미지 아이디, 상품이미지 파일 정보 전달
        }
        return item.getId();
    }
